...
topic_mode = {}  # per-user Topic caption mode


@bot.on_callback_query(filters.regex(r"^topic_(yes|no)$"))
async def topic_mode_callback(bot: Client, cq: CallbackQuery):
    try:
        user_id = cq.from_user.id
        enabled = cq.data == "topic_yes"
        topic_mode[user_id] = enabled
        status = "✅ YES" if enabled else "❌ NO"
        await cq.answer(f"Topic Mode: {status}", show_alert=False)
        await cq.message.edit_reply_markup(
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("📁 Topic: YES" + (" ✓" if enabled else ""), callback_data="topic_yes"),
                    InlineKeyboardButton("📁 Topic: NO" + (" ✓" if not enabled else ""), callback_data="topic_no"),
                ],
                [InlineKeyboardButton("📚 Help", url="https://t.me/ItsUGBot")]
            ])
        )
    except Exception as e:
        print(f"Topic callback error: {e}")
...
async def start(bot: Client, m: Message):
    try:
        if m.chat.type == "channel":
            ...
            topic_enabled = topic_mode.get(m.from_user.id, False)
            topic_status = "YES" if topic_enabled else "NO"
            ...
            await m.reply_photo(
                ...
                caption=(
                    ...
                    f"{commands_list}\n"
                    f"**📁 Topic Mode:** `{topic_status}`\n\n"
                    f"Choose Topic Mode below. This only changes the caption heading; it does not create Telegram topics/folders."
                ),
                reply_markup=InlineKeyboardMarkup([
                    [
                        InlineKeyboardButton("📁 Topic: YES" + (" ✓" if topic_enabled else ""), callback_data="topic_yes"),
                        InlineKeyboardButton("📁 Topic: NO" + (" ✓" if not topic_enabled else ""), callback_data="topic_no"),
                    ],
                    [InlineKeyboardButton("📚 Help", url="https://t.me/ItsUGBot")]
                ])
            )
...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:    
        ...
        links = []
        for i in content:
            # Supports: Math | Arithmetic | Percentage Class - 01 : https://example.com/video.mp4
            match = re.match(r"^(.*?)\s*:\s*(https?://\S+)\s*$", i)
            if match:
                ...
                # Existing downloader expects URL without the scheme.
                url = full_url.split("://", 1)[1]
                links.append([name, url, topic_name])
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            topic_name = links[i][2] if len(links[i]) > 2 else ""
            if "," in raw_text3:
